...
export default function Ticker() {
  ...
  const items = safeMatches.map((m) => {
    ...
    const label =
      m.status === "finished" ? "FINAL" : m.status === "live" ? "LIVE" : `${m.time} · ${m.court}`
    return {
      id: m.id,
      status: m.status,
  ...
  // No matches → render a static info bar (no marquee, no repetition)
  if (items.length === 0) {
    return (
      <div className="relative z-40 flex items-center overflow-hidden border-b border-white/10 bg-slate-950">
        ...
        <div className="flex w-full items-center px-6 py-1.5">
          <span className="inline-block rounded bg-gold-500/20 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-gold-400 mr-2">
            INFO
          </span>
          <span className="text-sm font-medium text-slate-200">
            IMRT 3x3 Championship 2026 • September 27 – October 3
          </span>
        </div>
      </div>
    )
  ...
  // Matches exist → scrolling marquee
