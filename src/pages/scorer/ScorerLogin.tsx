...
export default function ScorerLogin() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (username === SCORER_CREDENTIALS.username && password === SCORER_CREDENTIALS.password) {
        localStorage.setItem("imrt_scorer_auth", "true")
        navigate("/scorer")
      } else {
        setError("Invalid username or password. Try scorer / scorer123")
      }
      setLoading(false)
    }, 400)
  }
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-slate-900/90 p-8 shadow-2xl backdrop-blur-xl">
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <input
                ...
                placeholder="Enter scorer password"
                className="w-full rounded-xl border border-white/10 bg-slate-950 py-3 pl-10 pr-4 text-sm text-white placeholder-slate-600 focus:border-gold-500 focus:outline-none focus:ring-1 focus:ring-gold-500"
              />
            </div>
          </div>
        ...
        <div className="mt-6 rounded-xl border border-white/5 bg-slate-950/50 p-3 text-center text-xs text-slate-500">
          Default credentials: <code className="text-gold-400">scorer</code> / <code className="text-gold-400">scorer123</code>
        </div>
      </div>
    </div>
  )
